...
AttackFlow._doAttackAction = function () {
  ...
  if (actX == x && actY == y - 1) adj = true;
  if (actX == x && actY == y + 1) adj = true;
  if (actX == x - 1 && actY == y) adj = true;
  if (actX == x + 1 && actY == y) adj = true;
  if (SkillControl.getPossessionCustomSkill(passive, "Blademail")) {
    ...
    var i;
    if (skill.custom.all) suitable = true;
    for (i = 0; i < arr.length; i++) {
      if (actWeapon.getWeaponType().getName() == arr[i]) suitable = true;
    }
  ...
  for (i = 0; i < count; i++) {
    turnState = StateControl.arrangeState(
      active,
      activeStateArray[i],
...
RealBattle.doHitAction = function () {
  ...
  if (actX == x + 1 && actY == y) adj = true;
  if (
    SkillControl.getPossessionCustomSkill(battlerPassive.getUnit(), "Blademail")
  ) {
    var skill = SkillControl.getPossessionCustomSkill(
      battlerPassive.getUnit(),
      "Blademail"
    );
    var suitable = false;
    var arr = skill.custom.reflect;
...
EasyMapUnit._doHitAction = function () {
  ...
  var x = battlerPassive.getMapX();
  var y = battlerPassive.getMapY();
  var actX = battlerActive.getMapX();
  var actY = battlerActive.getMapY();
  ...
  if (actX == x + 1 && actY == y) adj = true;
  if (
    SkillControl.getPossessionCustomSkill(battlerPassive, "Blademail")
  ) {
    var skill = SkillControl.getPossessionCustomSkill(
      battlerPassive,
      "Blademail"
    ...
    if (skill.custom.melee) {
      if (damagePassive < battlerPassive.getHp() && suitable && adj) {
        root.log("Blademail");
        reflected = true;
      }
    } else if (damagePassive < battlerPassive.getHp() && suitable) {
      root.log("Blademail");
      reflected = true;
    }
  }
