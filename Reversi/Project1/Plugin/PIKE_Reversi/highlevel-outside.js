...
OutSideParts.Turn = defineObject(BaseOutSideParts,
{
	...
	_getOutSidePartsValue: function() {
		...
		return ReversiController._getWhiteValue(table);
	},
	
	_getPos: function() {
		var x = root.getGameAreaWidth();
		
		x = Math.floor(x / 2) - 70;

		y = root.getGameAreaHeight();
		
		return createPos(x, y - 20);
	}
}
);
...
OutSideParts.Bonus = defineObject(BaseOutSideParts,
{
	...
	_getOutSidePartsValue: function() {
		var table = ReversiController._getTable();
		return ReversiController._getBlackValue(table);
	},
